...
1. **会话内「账本 / 用量」两个 seg 视图是否纳入 MU-2**（本方案纳入首版，细化留 MU-3）。若要 MU-2 更薄，可整体推到 MU-3，seg 只留「对话」。
2. **流式（G2.2）**：MU-2 不接 delta，助手消息整块出现。是否接受？若要流式，需先做 M6 真模型方言核对（否则 faux 方言写的解析必被推翻）。
3. **工具集行（G4.5）**：计划显示「角色白名单 N」而非「生效工具集」。若要生效值，需 M6 前扩 `AgentSnapshot`（属协议扩展，超出本片）。
4. **S3 的「Agent」tab**：原型未画内容。建议先做「跨团队的类型清单 + 每类型引用它的团队数」，只读；点进去仍复用类型编辑器。若不要，可整个 tab 先隐藏。
5. **升级提议卡**：原型 `s2-solo.html:62-90` 的提议卡无触发源（没有「建议升级」事件）。本方案只做常驻入口、不做提议卡。是否接受？
